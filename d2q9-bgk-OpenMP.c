...
#pragma omp parallel
  {
#pragma omp for firstprivate(ii,w1,w2) private(jj)
...
#pragma omp for private(ii) private(jj) private(y_n) private(x_e) private(y_s) private(x_w)   
...
#pragma omp for private(ii) private(jj)
  for(ii=0;ii<params.ny;ii++) {
    for(jj=0;jj<params.nx;jj++) {
  ...
  ** mirroring, and writing into main grid */
  ...
  double inverselocal_density;         /* sum of densities in a particular cell */
...
#pragma omp for private(ii) private(jj) private(local_density) private(kk) private(u_x) private(u_y) private(u) private(inverselocal_density) private(u_sq) private(d_equ)
  for(ii=0;ii<params.ny;ii++) {
    for(jj=0;jj<params.nx;jj++) {
  ...
  d_equ[1] = w1 * local_density * (1.0 + (u[1] * inv_c_sq)
           + (((u[1] * u[1]) * inv_c_sq * inv_c_sq) / (2.0))
           - (u_sq * inv_c_sq) / (2.0));
  d_equ[2] = w1 * local_density * (1.0 + (u[2] * inv_c_sq)
           + (((u[2] * u[2]) * inv_c_sq * inv_c_sq) / (2.0))
           - (u_sq * inv_c_sq) / (2.0));
  d_equ[3] = w1 * local_density * (1.0 + (u[3] * inv_c_sq)
           + (((u[3] * u[3]) * inv_c_sq * inv_c_sq) / (2.0))
           - (u_sq * inv_c_sq) / (2.0));
  d_equ[4] = w1 * local_density * (1.0 + (u[4] * inv_c_sq)
           + (((u[4] * u[4]) * inv_c_sq * inv_c_sq) / (2.0))
           - (u_sq * inv_c_sq) / (2.0));
  /* diagonal speeds: weight w2 */
  d_equ[5] = w2 * local_density * (1.0 + (u[5] * inv_c_sq)
           + (((u[5] * u[5]) * inv_c_sq * inv_c_sq) / (2.0))
           - (u_sq * inv_c_sq) / (2.0));
  d_equ[6] = w2 * local_density * (1.0 + (u[6] * inv_c_sq)
           + (((u[6] * u[6]) * inv_c_sq * inv_c_sq) / (2.0))
           - (u_sq * inv_c_sq) / (2.0));
  d_equ[7] = w2 * local_density * (1.0 + (u[7] * inv_c_sq)
           + (((u[7] * u[7]) * inv_c_sq * inv_c_sq) / (2.0))
           - (u_sq * inv_c_sq) / (2.0));
  d_equ[8] = w2 * local_density * (1.0 + (u[8] * inv_c_sq)
           + (((u[8] * u[8]) * inv_c_sq * inv_c_sq) / (2.0))
           - (u_sq * inv_c_sq) / (2.0));
  /* relaxation step */
  for(kk=0;kk<NSPEEDS;kk++) {
...
#pragma omp parallel
{
#pragma omp for reduction(+:tot_u) reduction(+:tot_cells) private(u_x) private(u_y) private(jj) private(local_density) private(inverselocal_density) private(kk)
